...
검증 스크립트는 프로젝트 필수 진입 문서, 프로젝트 인덱스, 프로젝트 루트의 레거시 문서 디렉터리명, 공용 템플릿 경로, 상대 링크, 절대 사용자 경로 포함 여부, **phase PRD 파일명 규칙과 `docs/prd.md` 역참조**를 확인합니다. 링크 검사는 Git이 추적하거나 새로 추가된 비무시 Markdown만 대상으로 하며, 저장소 루트는 스크립트 위치를 기준으로 계산합니다.
...
`docs/prd.md`는 항상 Phase 0(현재 MVP)만 다룹니다. "20. 우선순위"의 `Future`나 "다음 범위와 향후 범위" 같은 섹션을 도메인이 다른 항목으로 계속 채우지 않습니다.

- 다음에 만들 범위가 현재 MVP와 **같은 도메인/주제**의 단순 확장이면 `docs/prd.md`의 `Future`/다음 범위 섹션에 짧게 남겨도 됩니다.
- 다음에 만들 범위가 **다른 도메인이나 기술 영역**(예: 텍스트 제품에 음성/영상 추가, 웹 제품에 모바일 앱 추가, 신규 외부 연동)이면 그 즉시 `docs/prd.md`의 Future 항목으로 묻어두지 말고 별도 파일 `docs/prd-phase-{n}-{주제-slug}.md`(`{n}`은 1부터 시작하는 정수, `{주제-slug}`는 영문 kebab-case)를 새로 만들어 분리합니다.
- 각 phase 파일은 `_workspace-docs/templates/prd.md` 구조를 따르고, 메타데이터에 자신이 몇 번째 phase인지와 `docs/prd.md`로 돌아가는 링크를 남깁니다.
- 누군가 "이 Future 항목 구현해"라고 요청해도, 대응하는 `docs/prd-phase-*.md` 파일이 아직 없으면 구현부터 시작하지 않습니다. 먼저 그 phase의 PRD 파일을 만들어 범위·가정·리스크를 정의하고 승인받은 뒤에 구현합니다.
- phase 파일 이름 규칙과 `docs/prd.md` 역참조 여부는 `scripts/validate-workspace.ps1`이 검증합니다.

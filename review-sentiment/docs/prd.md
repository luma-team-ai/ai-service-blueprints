...
| --- | --- | --- |
| 2026-06-18 | 최초 초안 | |
| 2026-06-19 | 트랜스포머 모델을 SKT KoBERT(`skt/kobert-base-v1`)에서 KLUE-BERT(`klue/bert-base`)로 확정, 본문 표시명 통일 | SKT KoBERT는 커스텀 토크나이저·버전 호환성 이슈로 구현 리스크가 높고, KLUE-BERT는 표준 경로로 동급 이상 성능(NSMC) 확보 가능 (§2.1 참고) |
...
| 버전 호환성 | transformers 버전 업그레이드 시 `position_ids` state_dict 누락, padding token 오설정 등 깨짐 이슈 다수 보고 (SKTBrain/KoBERT #31, #106 등) | 표준 transformers 경로, 동일 이슈 보고 없음 |
| NSMC 정확도 | 약 89.0~89.6% | 동급 이상(개별 평가 약 90%+), PRD §19 목표(0.88) 충족 |
| Colab/Streamlit Cloud 재현성 | 런타임 패키지 버전을 완전히 통제할 수 없는 환경에서 깨질 위험(§22 리스크) | 표준 경로라 재현성 높음 |

결론: 주제·구조(3개 모델 비교, LIME 적용 방식) 변경 없이 트랜스포머 슬롯의 구현 리스크만 낮춘다. 이에 따라 본문의 "KoBERT" 표기는 모두 "KLUE-BERT"로 통일하며, SKT KoBERT 언급은 비교·불채택 근거를 설명하는 곳(§21, §25)에서만 유지한다.
...
| 기존 Mooditree UI | ML 모델 없음, "AI 프로젝트" 이름만 있음 | 실제 KLUE-BERT/LSTM 모델로 감정 분석 + 시각화 |
...
| KLUE-BERT 모델 | `pytorch_model.bin` | klue/bert-base (full fine-tune) |
...
| --- | --- | --- | --- | --- |
| KLUE-BERT를 Google Colab에서 fine-tuning 가능 | High (표준 transformers 경로, SKT KoBERT 대비 호환성 리스크 낮음 — §2.1.1) | Colab GPU 메모리 부족 시 CPU fallback 또는 LSTM만 데모 | 김관영 | Colab GPU 확인 후 대체 계획 수립 |
| Streamlit Cloud 무료 티어로 모델 3개 로드 가능 | Medium | 메모리 초과로 KLUE-BERT 로드 실패 가능 | 김관영 | 모델 경량화 또는 조건부 로드 구현 |
...
| 리스크 | 영향 | 확률 | 대응 |
| --- | --- | --- | --- |
| KLUE-BERT 학습에 GPU 시간 부족 | KLUE-BERT 누락, 2개 모델만 비교 | 중 | Colab Pro trial 또는 CPU 경량 모델 fallback |
...
- KLUE Benchmark 논문 (Park et al., 2021): https://arxiv.org/abs/2105.09680 — KLUE-BERT 성능·벤치마크 근거
- SKT KoBERT Hugging Face: https://huggingface.co/skt/kobert-base-v1 (불채택 — 커스텀 토크나이저, §2.1.1 비교 근거)
- SKTBrain/KoBERT GitHub Issues (호환성 이슈 사례): https://github.com/SKTBrain/KoBERT/issues/31, https://github.com/SKTBrain/KoBERT/issues/106
- 한국어 PLM 비교 서베이 (NSMC 정확도 비교 출처): https://arxiv.org/pdf/2112.03014
